class ClothingsController < ApplicationController
  ...
  def index
    ...
    @clothings = @clothings.where(["first_name LIKE :filter OR last_name LIKE :filter", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?
  ...
  private
    ...
    def clothing_params
      params.require(:clothing).permit(:presenter_id, :transmission_id, :garment_id)
    end
end
